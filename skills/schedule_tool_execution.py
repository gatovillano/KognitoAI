...
class ScheduleToolExecutionInput(BaseModel):
    ...
    """
    tool_name: str = Field(
        description="Nombre de la herramienta a programar (ej: 'daily_analysis', 'daily_insights', 'weekly_cleanup')"
    )
...
class ScheduleToolExecutionTool(BaseTool):
    ...
    Herramientas disponibles para programar:
    - 'daily_analysis': Análisis diario de conocimiento
    - 'daily_insights': Generación diaria de insights proactivos
    ...
    async def _arun(
        ...
        **kwargs: Any
        ...
        try:
            ...
            available_tools = {
                'daily_analysis': self._get_daily_analysis_function,
                'daily_insights': self._get_daily_insights_function,
    ...
    def _get_daily_insights_function(self):
