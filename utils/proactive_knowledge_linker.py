...
async def get_gemini_model() -> ChatGoogleGenerativeAI:
    ...
    if _gemini_model is None:
        ...
        _gemini_model = ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=0.2)
...
async def analyze_relationship_with_llm(item_a: Dict[str, Any], item_b: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    prompt = f"""
    Eres un analista experto. Analiza la relación entre Ítem A y Ítem B.
    ...
    Clasifica la relación en "Duplicidad", "Sinergia", "Evolución", "Contradicción" o "Sin Relación Significativa".
