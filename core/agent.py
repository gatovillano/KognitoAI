...
async def create_and_run_agent(
    ...
    if len(full_history) < 2:
        ...
        if prev_threads:
            ...
            if prev_messages:
                ...
                summarization_chain = summarization_prompt | llm_for_summary if llm_for_summary else summarization_prompt
    ...
    profile_info = []
    custom_prompt_from_profile = None # Renombrado para mayor claridad
    if user_profile:
        ...
        custom_prompt_from_profile = user_profile.system_prompt
    ...
    # --- Lógica de construcción del prompt de sistema final ---
    final_system_prompt_content = settings.default_system_prompt # Siempre empezamos con el prompt por defecto
    logger.debug(f"DEBUG: Prompt base (config.py): {final_system_prompt_content[:200]}...")

    # Añadir prompt del perfil si existe
    if custom_prompt_from_profile:
        logger.info("Añadiendo prompt personalizado del perfil de usuario.")
        # Reemplazar referencias a variables problemáticas como row['energy'] o index
        processed_custom_prompt = custom_prompt_from_profile.replace("row['energy']", "valor_energia").replace("index", "indice")
        final_system_prompt_content += f"\n\n--- Instrucciones Adicionales del Perfil del Usuario ---\n{processed_custom_prompt}"
        logger.debug(f"DEBUG: Prompt después de perfil: {final_system_prompt_content[:200]}...")

    # Añadir prompt del workspace si existe
    if workspace_id:
        async with DBSession(SessionLocal) as db:
            workspace = await db.get(Workspace, uuid.UUID(workspace_id))
            if workspace and workspace.system_prompt:
                logger.info(f"Añadiendo prompt de sistema del workspace {workspace_id}.")
                final_system_prompt_content += f"\n\n--- Instrucciones Adicionales del Workspace ---\n{workspace.system_prompt}"
                logger.debug(f"DEBUG: Prompt después de workspace: {final_system_prompt_content[:200]}...")
    
    # --- FIX: Pre-formatear el prompt final para eliminar placeholders inválidos ---
    # Esto se aplica al prompt *final* que ya incluye todas las adiciones.
    try:
        final_system_prompt_content = final_system_prompt_content.format(
            query=user_message,
            web_summary="",
            relevant_memories=relevant_memories,
            # Asegúrate de que todas las variables usadas en el prompt final estén aquí
        )
    except KeyError as e:
        logger.warning(f"No se pudo pre-formatear el prompt final, puede que contenga placeholders desconocidos: {e}")
        final_system_prompt_content = final_system_prompt_content.replace('{query}', user_message)
        final_system_prompt_content = final_system_prompt_content.replace('{web_summary}', '')
        ...
        if problematic_placeholder in final_system_prompt_content:
            final_system_prompt_content = final_system_prompt_content.replace(problematic_placeholder, relevant_memories)
    logger.debug(f"DEBUG: Prompt final después de pre-formateo: {final_system_prompt_content[:200]}...")

    all_tools = get_all_langchain_tools(account_id=account_id, telegram_id=str(telegram_id) if telegram_id is not None else "")
    tools = all_tools
    ...
    system_prompt_parts = [
        ...
        summary_string,
        "<hr>",
        final_system_prompt_content, # Usar la nueva variable aquí
        "<hr>",
    ...
    logger.debug(f"DEBUG: system_prompt_content final (después de concatenar): {system_prompt_content[:200]}...") # Log final antes de pasar al SystemMessage
    ...
    if not main_llm_instance:
        raise RuntimeError("El LLM principal no está inicializado.")
    main_llm: BaseChatModel = cast(BaseChatModel, main_llm_instance)
    ...
    main_llm_instance = get_main_llm()
    if main_llm_instance:
        main_llm: BaseChatModel = cast(BaseChatModel, main_llm_instance)
        if main_llm.get_num_tokens_from_messages(updated_full_history) > 3000:
