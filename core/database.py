...
# ==============================================================================
# SECCIÓN: MODELOS DE CORREO ELECTRÓNICO (Email Inbox Module)
# ==============================================================================

class EmailAccount(Base):
    """
    Representa una cuenta de correo electrónico configurada por el usuario.
    Soporta autenticación IMAP/SMTP, OAuth2 (Gmail, Outlook), y App Passwords.
    """
    ...
    # Información básica
    name = Column(String(255), nullable=False, comment="Nombre amigable para la cuenta (ej. 'Personal Gmail')")
    email_address = Column(String(255), nullable=False, index=True, comment="Dirección de correo electrónico")
    provider = Column(String(50), nullable=True, comment="Proveedor: gmail, outlook, yahoo, disroot, generic")
    
    # Configuración IMAP
    imap_host = Column(String(255), nullable=True, comment="Host IMAP")
    imap_port = Column(Integer, nullable=True, default=993, comment="Puerto IMAP")
    imap_use_ssl = Column(Boolean, default=True, nullable=False, comment="Usar SSL/TLS para IMAP")
    
    # Configuración SMTP
    smtp_host = Column(String(255), nullable=True, comment="Host SMTP")
    smtp_port = Column(Integer, nullable=True, default=587, comment="Puerto SMTP")
    smtp_use_tls = Column(Boolean, default=True, nullable=False, comment="Usar STARTTLS para SMTP")
    smtp_use_ssl = Column(Boolean, default=False, nullable=False, comment="Usar SSL para SMTP (puerto 465)")
    
    # Autenticación
    auth_type = Column(String(50), nullable=False, default="password", comment="Tipo de auth: password, oauth2, app_password")
    username = Column(String(255), nullable=True, comment="Usuario IMAP/SMTP (puede ser diferente al email)")
    
    # Credenciales cifradas (usando pgcrypto como UserSecret)
    encrypted_password = Column(Text, nullable=True, comment="Contraseña o App Password cifrada con pgcrypto")
    
    # OAuth2 tokens (cifrados)
    encrypted_access_token = Column(Text, nullable=True, comment="OAuth2 access token cifrado")
    encrypted_refresh_token = Column(Text, nullable=True, comment="OAuth2 refresh token cifrado")
    token_expires_at = Column(DateTime(timezone=True), nullable=True, comment="Expiración del access token")
    oauth_scopes = Column(JSONB, nullable=True, comment="Scopes OAuth2 concedidos")
    
    # Estado y configuración
    is_active = Column(Boolean, default=True, nullable=False, comment="Cuenta activa para sincronización")
    is_default = Column(Boolean, default=False, nullable=False, comment="Cuenta por defecto para enviar")
    sync_enabled = Column(Boolean, default=True, nullable=False, comment="Sincronización automática habilitada")
    sync_interval_minutes = Column(Integer, default=15, nullable=False, comment="Intervalo de sincronización en minutos")
    last_sync_at = Column(DateTime(timezone=True), nullable=True, comment="Última sincronización exitosa")
    last_sync_error = Column(Text, nullable=True, comment="Error de la última sincronización")
    
    # Carpetas sincronizadas (JSON con nombres de carpetas IMAP)
    synced_folders = Column(JSONB, nullable=True, server_default=text("'[]'::jsonb"), comment="Lista de carpetas a sincronizar")
    
    created_at = Column(DateTime(timezone=True), server_default=text("CURRENT_TIMESTAMP"))
    updated_at = Column(DateTime(timezone=True), server_default=text("CURRENT_TIMESTAMP"), onupdate=text("CURRENT_TIMESTAMP"))

    # Relaciones
    account = relationship("Account", backref=backref("email_accounts", cascade="all, delete-orphan"))
    emails = relationship("Email", back_populates="email_account", cascade="all, delete-orphan")
    folders = relationship("EmailFolder", back_populates="email_account", cascade="all, delete-orphan")

    __table_args__ = (
        UniqueConstraint('account_id', 'email_address', name='_account_email_uc'),
        Index('ix_email_accounts_account_id', account_id),
        Index('ix_email_accounts_email_address', email_address),
    )
...
class EmailFolder(Base):
    """
    Representa una carpeta/label IMAP (INBOX, Sent, Drafts, Spam, etc.)
    """
    __tablename__ = "email_folders"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    email_account_id = Column(UUID(as_uuid=True), ForeignKey("email_accounts.id", ondelete='CASCADE'), nullable=False, index=True)
    
    # Información de la carpeta IMAP
    imap_name = Column(String(255), nullable=False, comment="Nombre real en IMAP (ej. 'INBOX', '[Gmail]/Sent Mail')")
    display_name = Column(String(255), nullable=False, comment="Nombre para mostrar en UI")
    folder_type = Column(String(50), nullable=True, comment="Tipo: inbox, sent, drafts, spam, trash, archive, custom")
    
    # Estado
    is_subscribed = Column(Boolean, default=True, nullable=False, comment="Suscrita en IMAP")
    is_selectable = Column(Boolean, default=True, nullable=False, comment="Se puede seleccionar (no es solo contenedor)")
    message_count = Column(Integer, default=0, nullable=False, comment="Número de mensajes")
    unseen_count = Column(Integer, default=0, nullable=False, comment="Número de mensajes no leídos")
    
    # Jerarquía
    parent_id = Column(UUID(as_uuid=True), ForeignKey("email_folders.id", ondelete='CASCADE'), nullable=True, index=True)
    delimiter = Column(String(10), nullable=True, comment="Delimitador IMAP (ej. '/', '.')")
    
    created_at = Column(DateTime(timezone=True), server_default=text("CURRENT_TIMESTAMP"))
    updated_at = Column(DateTime(timezone=True), server_default=text("CURRENT_TIMESTAMP"), onupdate=text("CURRENT_TIMESTAMP"))

    # Relaciones
    email_account = relationship("EmailAccount", back_populates="folders")
    emails = relationship("Email", back_populates="folder")
    subfolders = relationship("EmailFolder", backref=backref("parent", remote_side=[id]), cascade="all, delete-orphan")

    __table_args__ = (
        UniqueConstraint('email_account_id', 'imap_name', name='_account_folder_imap_name_uc'),
        Index('ix_email_folders_account_id', email_account_id),
        Index('ix_email_folders_parent_id', parent_id),
    )

    def __repr__(self):
        return f"<EmailFolder(id={self.id}, name={self.display_name}, type={self.folder_type})>"


class Email(Base):
    """
    Representa un mensaje de correo electrónico individual.
    """
    __tablename__ = "emails"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    email_account_id = Column(UUID(as_uuid=True), ForeignKey("email_accounts.id", ondelete='CASCADE'), nullable=False, index=True)
    folder_id = Column(UUID(as_uuid=True), ForeignKey("email_folders.id", ondelete='SET NULL'), nullable=True, index=True)
    
    # Identificadores IMAP
    imap_uid = Column(String(255), nullable=False, index=True, comment="UID IMAP del mensaje")
    message_id = Column(String(512), nullable=True, index=True, comment="Message-ID header (global unique)")
    in_reply_to = Column(String(512), nullable=True, index=True, comment="In-Reply-To header para threading")
    references = Column(JSONB, nullable=True, comment="References header como array para threading")
    
    # Cabeceras principales
    subject = Column(Text, nullable=True, comment="Asunto del correo")
    from_address = Column(String(512), nullable=True, comment="Remitente (From header)")
    from_name = Column(String(255), nullable=True, comment="Nombre del remitente")
    to_addresses = Column(JSONB, nullable=True, comment="Destinatarios To como array de {email, name}")
    cc_addresses = Column(JSONB, nullable=True, comment="Destinatarios CC como array")
    bcc_addresses = Column(JSONB, nullable=True, comment="Destinatarios BCC como array (solo salientes)")
    reply_to = Column(String(512), nullable=True, comment="Reply-To header")
    
    # Fechas
    date_sent = Column(DateTime(timezone=True), nullable=True, index=True, comment="Fecha del header Date")
    date_received = Column(DateTime(timezone=True), server_default=text("CURRENT_TIMESTAMP"), index=True, comment="Fecha de recepción en nuestro sistema")
    
    # Contenido
    body_text = Column(Text, nullable=True, comment="Cuerpo en texto plano")
    body_html = Column(Text, nullable=True, comment="Cuerpo en HTML")
    snippet = Column(Text, nullable=True, comment="Vista previa corta para lista")
    
    # Flags/Estado
    is_read = Column(Boolean, default=False, nullable=False, index=True, comment="Marcado como leído")
    is_flagged = Column(Boolean, default=False, nullable=False, comment="Marcado con bandera/estrella")
    is_answered = Column(Boolean, default=False, nullable=False, comment="Respondido")
    is_draft = Column(Boolean, default=False, nullable=False, comment="Es borrador")
    is_deleted = Column(Boolean, default=False, nullable=False, comment="Marcado para eliminación (IMAP \\Deleted)")
    is_spam = Column(Boolean, default=False, nullable=False, comment="Marcado como spam")
    
    # Adjuntos
    has_attachments = Column(Boolean, default=False, nullable=False, comment="Tiene adjuntos")
    attachment_count = Column(Integer, default=0, nullable=False, comment="Número de adjuntos")
    total_attachment_size = Column(Integer, default=0, nullable=False, comment="Tamaño total adjuntos en bytes")
    
    # Threading
    thread_id = Column(String(512), nullable=True, index=True, comment="ID de hilo para agrupación")
    
    # Tamaño
    size_bytes = Column(Integer, nullable=True, comment="Tamaño del mensaje en bytes")
    
    # Metadatos adicionales
    headers = Column(JSONB, nullable=True, comment="Cabeceras completas como JSON")
    labels = Column(JSONB, nullable=True, comment="Labels/Gmail categories (Promotions, Social, etc.)")
    ...
    __table_args__ = (
        UniqueConstraint('email_account_id', 'imap_uid', name='_account_imap_uid_uc'),
        Index('ix_emails_account_folder', email_account_id, folder_id),
        Index('ix_emails_date_sent', date_sent),
    ...
    def __repr__(self):
        return f"<Email(id={self.id}, subject={self.subject[:50] if self.subject else 'None'}, from={self.from_address})>"
...
class EmailAttachment(Base):
    ...
    filename = Column(String(512), nullable=True, comment="Nombre del archivo")
    content_type = Column(String(255), nullable=True, comment="MIME type (ej. application/pdf)")
    content_id = Column(String(512), nullable=True, comment="Content-ID para inline images (cid:)")
    content_disposition = Column(String(50), nullable=True, comment="attachment o inline")
    
    # Tamaño
    size_bytes = Column(Integer, nullable=True, comment="Tamaño en bytes")
    
    # Almacenamiento
    storage_path = Column(String(1024), nullable=True, comment="Ruta local o S3 donde se guarda el archivo")
    is_inline = Column(Boolean, default=False, nullable=False, comment="Es imagen inline (cid:)")
    
    # Hash para deduplicación
    content_hash = Column(String(64), nullable=True, comment="SHA256 del contenido")
    ...
    __table_args__ = (
        ...
        Index('ix_email_attachments_content_hash', content_hash),
    )
