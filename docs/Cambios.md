...
## 16-12-2025 Mejora de la Carga y Movimiento de Grafos de Conocimiento (Frontend)
 
 Se han realizado ajustes adicionales en la visualización del grafo para mejorar la fluidez y reducir la carga computacional en el navegador.
 
- **Ajustes de Física**: Se refinaron los parámetros de `barnesHut` en `src/components/KnowledgeGraph/GraphVisualization.tsx` para una estabilización más rápida.
- **Gestión de Eventos**: Se optimizó la forma en que se manejan los eventos de clic y hover para evitar re-renderizados innecesarios.
...
- **Modificación del Diálogo de Eventos (Frontend)**:
  - Se añadió un botón "Configurar Recordatorio" en el diálogo de edición/creación de eventos (`src/app/(dashboard)/agenda/event-dialog.tsx`). This button opens a new specific dialog for the reminder.
...
- **Notificaciones Toast (Frontend)**:
  - Se utilizan `toast.success` and `toast.error` para proporcionar retroalimentación visual al usuario sobre la configuración exitosa o fallida del recordatorio.
...
## 19-12-2025 Resolución de Dependencia Circular en el Agente de Investigación Profunda
 
 Se ha resuelto una dependencia circular entre `core/utils/tool_utils.py` and `core/tools.py` que impedía la correcta inicialización del agente de investigación profunda.
 
- **Problema**: `core/utils/tool_utils.py` importaba `get_all_langchain_tools` de `core/tools.py`, mientras que `core/tools.py` (a través de `tools/deep_research_tool.py` y `core/agents/deep_researcher.py`) también dependía de `core/utils/tool_utils.py`, creando un ciclo de importación.
- **Solución**:
  - Se modificó `core/utils/tool_utils.py`:
    - Se eliminó la importación de `get_all_langchain_tools`.
    - Se eliminaron las cachés globales `_all_tools_cache` y `_individual_tool_cache`.
    - La función `get_tool_by_name` ahora acepta una lista `all_tools: List[BaseTool]` como argumento, y busca la herramienta dentro de esta lista.
  - Se modificó `core/agents/deep_researcher.py`:
    - Se eliminó la función auxiliar local `async def get_all_tools(config: RunnableConfig)`.
    - En las funciones `researcher` y `researcher_tools`, se añadió la importación de `get_all_langchain_tools` de `core.tools`.
    - Se llama a `get_all_langchain_tools` para obtener la lista completa de herramientas.
    - Se utiliza esta lista para construir un diccionario `tools_by_name` y se pasa a `get_tool_by_name` (aunque en este caso, `get_tool_by_name` ya no es estrictamente necesaria dentro de `deep_researcher.py` ya que se construye un `tools_by_name` dict directamente).
- **Impacto**: Se ha roto la dependencia circular, permitiendo que el agente de investigación profunda se inicialice y funcione correctamente. La gestión de herramientas ahora es más modular y robusta.
 
 ---
 
 
## 19-12-2025 Corrección de `TypeError` en `tool_node` del Agente Principal
 
 Se ha corregido un `TypeError` en la función `tool_node` de `core/agent.py` que ocurría porque `get_tool_by_name` esperaba un argumento `all_tools` que no se le estaba proporcionando.
 
- **Problema**: Después de refactorizar `core/utils/tool_utils.py`, la función `get_tool_by_name` ahora requiere una lista de todas las herramientas (`all_tools`) como argumento. La función `tool_node` en `core/agent.py` no estaba pasando este argumento, lo que resultaba en un `TypeError`.
- **Solución**:
  - Se modificó la función `tool_node` en `core/agent.py`.
  - Se añadió una llamada a `get_all_langchain_tools` (importada de `core.tools`) dentro de `tool_node` para obtener la lista completa de herramientas.
  - Esta lista (`all_tools`) ahora se pasa como argumento a `get_tool_by_name`.
  - Se ajustó la forma en que se obtienen `telegram_id` y `workspace_id` de la configuración para asegurar la compatibilidad.
- **Impacto**: El agente principal ahora puede invocar herramientas correctamente sin errores de tipo, asegurando el flujo normal de la conversación y la ejecución de herramientas.
 
 ---
 
 
## 19-12-2025 Corrección de `TypeError` en `get_all_langchain_tools`
 
 Se ha corregido un `TypeError` en la función `get_all_langchain_tools` de `core/tools.py` que ocurría porque se le pasaba un argumento `workspace_id` inesperado.
 
- **Problema**: La función `get_all_langchain_tools` en `core/tools.py` no estaba definida para aceptar el argumento `workspace_id`, lo que causaba un `TypeError` cuando se invocaba desde `core/agent.py` o `core/agents/deep_researcher.py`.
- **Solución**:
  - Se modificó la firma de la función `get_all_langchain_tools` en `core/tools.py` para incluir `workspace_id: Optional[str] = None`.
  - Se actualizó la función auxiliar `_instantiate_tool` para aceptar `workspace_id` y pasarlo a los constructores de herramientas que lo requieran (como `GitHubRepoTool` y las herramientas estándar).
  - Se añadió la inyección de `workspace_id` en la instanciación de `DeepResearchTool` para asegurar que reciba todos los parámetros de contexto necesarios.
- **Impacto**: La función `get_all_langchain_tools` ahora es compatible con los argumentos que se le pasan, resolviendo el `TypeError` y permitiendo que todas las herramientas se instancien correctamente con el contexto adecuado.
 
 ---
 
 
## 19-12-2025 Corrección de Declaración Duplicada de `DeepResearchTool` y Manejo de Dependencias
 
 Se ha corregido un error de `Duplicate function declaration` para la herramienta `deep_research` y se ha asegurado la correcta instanciación de sus dependencias.
 
- **Problema**: La herramienta `DeepResearchTool` se estaba instanciando dos veces, una a través de `tool_classes_to_instantiate` y otra en un bloque `try` separado, lo que causaba una declaración duplicada. Además, `DeepResearchTool` requiere que `web_search_tool` and `add_web_to_rag_tool` sean inyectadas como dependencias en su constructor, lo cual no es posible si se instancia directamente desde `tool_classes_to_instantiate`.
- **Solución**:
  - Se eliminó `DeepResearchTool` de la lista `tool_classes_to_instantiate` en `core/tools.py`.
  - Se restauró y mantuvo el bloque `try` en `get_all_langchain_tools` que se encarga de:
    - Instanciar `AddWebToRAGTool`.
    - Localizar la instancia de `web_search_tool` (que es una herramienta de fábrica).
    - Instanciar `DeepResearchTool` pasando `web_search_tool` and `add_web_to_rag_tool` como dependencias.
- **Impacto**: Se resuelve el error de declaración duplicada y se garantiza que `DeepResearchTool` se instancie correctamente con todas sus dependencias, permitiendo que las herramientas se vinculen al modelo de lenguaje y que el agente funcione sin interrupciones.
 
 ---
 
 
## 20-12-25 Implementación de Diálogo de Detalles en Perfiles
 
 Se ha implementado una nueva funcionalidad en el módulo de Perfiles que permite visualizar los detalles de un contacto y sus elementos vinculados mediante un diálogo modal, mejorando la experiencia de usuario al evitar navegaciones innecesarias.
 
- **Nuevo Componente `ProfileDetailDialog`**: Se creó un componente especializado para la visualización de datos del perfil (email, teléfono, etiquetas, campos personalizados) y objetos vinculados (notas, eventos de agenda, tareas y álbumes).
- **Actualización de `ProfilesPage`**: Se modificó el evento `onClick` de las tarjetas de perfil para activar el nuevo diálogo en lugar de redirigir a la página de detalles.
- **Integración de Flujos**: Se añadió un botón de "Editar" dentro del diálogo de detalles que abre el formulario de edición existente, manteniendo la coherencia del sistema.
- **Optimización de UI**: Se utilizaron componentes de `shadcn/ui` como `ScrollArea`, `Badge`, `Separator` and `Dialog` para garantizar una estética premium y funcional.
 
 ---
 
 
## 20-12-2025 Corrección en Conteo de Memorias del Agente
 
 Se ha corregido una inconsistencia en el conteo de memorias del agente. El sistema reportaba "0 memorias guardadas" a pesar de que el agente sí las reconocía y utilizaba. El problema se debía a que las funciones de estadísticas solo contaban un tipo de memoria (`document_chunk`) e ignoraban otras como las memorias proactivas (`user_memory_proactive_llm`) y los resúmenes de hilos (`thread_summary`).
 
- **Modificación de consultas SQL**: Se actualizaron las consultas en las funciones `get_graph_status` and `get_graph_stats` del archivo `api/knowledge_graph.py` para incluir los tipos de contenido `user_memory_proactive_llm` and `thread_summary` en el conteo.
- **Impacto**: El conteo de memorias ahora es preciso y refleja todos los tipos de memoria que el agente almacena, eliminando la inconsistencia reportada por el usuario.
 
 ---
 
 
## 21-12-25 Robustez en Tool Calling y Validación de Argumentos
 
 Se ha implementado una mejora significativa en la robustez del sistema de llamadas a herramientas (tool calling) para mitigar errores comunes en modelos de lenguaje menos precisos (como los disponibles vía OpenRouter), asegurando que las herramientas siempre reciban los parámetros necesarios.
 
- **Validación Dual Pydantic (v1/v2)**: Se actualizó la lógica de validación en `core/agent.py` para soportar tanto Pydantic v1 (usado por herramientas como `add_note`) como Pydantic v2. Esto permite detectar la falta de campos obligatorios (como `content`) antes de intentar ejecutar la herramienta.
- **Normalización de Argumentos**: Se añadió lógica en `call_model_node` para normalizar los argumentos de las herramientas, manejando casos donde el LLM envía un JSON string en lugar de un objeto, o confunde las claves `args` y `arguments`.
- **Feedback Correctivo al LLM**: En lugar de fallar con una excepción, el sistema ahora captura los errores de validación y devuelve un `ToolMessage` detallado al modelo con instrucciones claras sobre cómo corregir la llamada.
- **Refuerzo de Prompts**: Se añadieron instrucciones críticas en `KAI_SYSTEM_PROMPT` (`core/prompts.py`) prohibiendo explícitamente las llamadas a herramientas con argumentos incompletos y ordenando al modelo preguntar al usuario si falta información obligatoria.
- **Filtrado de Llamadas Inválidas**: Se mejoró el filtrado de llamadas a herramientas "basura" (sin nombre o vacías) que algunos modelos generan erróneamente.
 
 ---
 
 
## 21-12-25 Mejora del Binding de Herramientas para OpenRouter/LiteLLM
 
 Se ha optimizado la forma en que las herramientas se vinculan a los modelos de lenguaje para asegurar que todos los proveedores (especialmente aquellos a través de OpenRouter) reciban correctamente los esquemas de parámetros obligatorios.
 
- **Conversión Explícita a Formato OpenAI**: Se reemplazó el uso de `bind_tools` genérico por `convert_to_openai_tool` de forma explícita en `core/agent.py`. Esto garantiza que el JSON Schema generado sea el estándar más compatible, preservando correctamente la propiedad `required` de los argumentos.
- **Uso de Parámetros Nativos de LiteLLM**: Se modificó la vinculación para usar `.bind(tools=...)`, que es el formato que LiteLLM traduce de forma más fiable para proveedores externos.
- **Depuración de Esquemas**: Se añadió un log de depuración que imprime el esquema JSON de herramientas críticas (como `add_note`) antes de enviarlas al modelo, permitiendo verificar la integridad de las instrucciones técnicas en tiempo real.
 
 ---
 
 
## 21-12-25 Unificación de Pydantic v2 y Corrección de ToolMessage
 
 Se ha completado la migración de todas las herramientas del sistema a Pydantic v2 y se ha corregido un error crítico en el historial de mensajes de herramientas que confundía a los modelos de OpenRouter.
 
- **Migración Masiva a Pydantic v2**: Se actualizaron todas las herramientas en la carpeta `tools/` (incluyendo `add_note`, `schedule_event`, `memory_add`, etc.) para usar `pydantic` v2 nativo en lugar de la capa de compatibilidad `pydantic.v1`. Esto unifica el sistema de validación y mejora la precisión de los esquemas JSON enviados a los LLMs.
- **Corrección de ToolMessage sin nombre**: Se solucionó un error en `core/agent.py` donde los mensajes de error de herramientas se enviaban sin el atributo `name`. Esto causaba el warning `Found ToolMessage with missing/empty name` y rompía la lógica de seguimiento de herramientas en modelos sensibles como los de OpenRouter.
- **Simplificación de Validación**: Se eliminó la lógica dual de validación (v1/v2) en el `tool_node` de `core/agent.py`, dejando una implementación limpia y eficiente basada exclusivamente en Pydantic v2.
- **Optimización de Mensajes de Error**: Los mensajes de retroalimentación al modelo ahora son más técnicos y concisos, facilitando que el LLM corrija sus llamadas a herramientas de forma inmediata sin perderse en explicaciones extensas.
 
 ---
 
 
## 21-12-25 Máxima Compatibilidad de Herramientas (Binding Dual y Refuerzo)
 
 Se ha implementado una capa de compatibilidad avanzada para garantizar que los modelos de OpenRouter y otros proveedores externos proporcionen argumentos válidos en sus llamadas a herramientas.
 
- **Binding Dual (tools + functions)**: Se modificó `core/agent.py` para enviar las herramientas simultáneamente en los formatos `tools` y `functions`. Esto resuelve problemas de interoperabilidad con modelos que aún dependen del formato legacy de OpenAI.
- **Refuerzo de Instrucciones Técnicas**: Se añadió una instrucción técnica crítica al final del prompt del sistema para modelos no-Gemini. Esta instrucción actúa como un recordatorio de "última milla", enfatizando la obligatoriedad de incluir todos los argumentos en el objeto `args`.
- **Normalización de Esquemas**: Se aseguró que la conversión a JSON Schema mantenga la propiedad `required` de forma estricta, forzando a los modelos a reconocer los parámetros obligatorios.
 
 ---
 
 
## 21-12-25 Corrección de SyntaxError: unmatched ']' en deep_researcher.py
 
 Se ha corregido un `SyntaxError: unmatched ']'` en el archivo `core/agents/deep_researcher.py` que impedía el correcto inicio de la aplicación.
 
- **Problema**: Un corchete `]` extra y sin emparejar se encontraba en la línea 319 de `core/agents/deep_researcher.py`, causando un error de sintaxis durante la carga del módulo.
- **Solución**: El corchete `]` extrínseco fue eliminado del archivo. Se verificó que el archivo ya no contenía el error, lo que sugiere que fue corregido por un proceso externo o una intervención manual previa.
- **Impacto**: La aplicación ahora puede iniciarse correctamente sin el `SyntaxError` en `deep_researcher.py`.
 
 ---
 
 
## 21-12-25 Corrección de ImportError: 'get_model_token_limit' en deep_researcher_utils.py
 
 Se ha corregido un `ImportError` en el archivo `core/agents/deep_researcher_utils.py` que impedía el correcto inicio de la aplicación.
 
- **Problema**: La función `get_model_token_limit` se importaba de `core/utils/llm_utils.py` en `core/agents/deep_researcher_utils.py`, pero esta función no existía en `core/utils/llm_utils.py` y tampoco se utilizaba en `deep_researcher_utils.py`.
- **Solución**: Se eliminó la importación de `get_model_token_limit` de la línea 33 de `core/agents/deep_researcher_utils.py`, ya que era una importación no utilizada y causaba el error.
- **Impacto**: La aplicación ahora puede iniciarse correctamente sin el `ImportError` y el código es más limpio al eliminar una importación innecesaria.
 
 ---
 
## 21-12-25 Implementación del Agente de Investigación Profunda con Tavily
 
Se ha refactorizado la implementación del agente de investigación profunda (`core/agents/deep_researcher.py`) para utilizar un modelo más complejo y de alto rendimiento, similar al de la implementación de referencia `open_deep_research`, integrando la API de Tavily para búsquedas web avanzadas.

- **Refactorización Completa**: Se ha reescrito `core/agents/deep_researcher.py` siguiendo la arquitectura de referencia, incluyendo un supervisor que delega tareas a investigadores especializados, y un proceso de clarificación de usuario.
- **Configuración Avanzada**: Se actualizó `core/agents/deep_researcher_config.py` para incluir la enumeración `SearchAPI` y campos para la configuración de modelos de resumen, investigación y reporte final, así como la configuración de MCP.
- **Estados Detallados**: Se actualizó `core/agents/deep_researcher_state.py` con nuevas clases `Summary`, `ClarifyWithUser`, `ResearchQuestion` y `ResearcherOutputState`, y se ajustaron los tipos de estado para `AgentState`, `SupervisorState` y `ResearcherState`.
- **Prompts Refinados**: Se actualizaron `core/agents/deep_researcher_prompts.py` con prompts más detallados y estructurados para la clarificación de usuario, la transformación de mensajes en temas de investigación, la supervisión, la investigación y la generación de reportes.
- **Utilidades Centralizadas**: Se creó `core/agents/deep_researcher_utils.py` para encapsular funciones de utilidad como la búsqueda con Tavily, el resumen de páginas web, el manejo de herramientas y la extracción de notas.
- **Integración de Tavily**: El agente ahora utiliza la API de Tavily para realizar búsquedas web, lo que permite obtener resultados más completos y precisos.
- **Manejo de Errores y Tipado**: Se corrigieron numerosos errores de tipado y se implementaron `casts` explícitos para asegurar la compatibilidad con LangChain y LangGraph, y se añadió la inicialización de LLMs en el bloque `if __name__ == "__main__":` para pruebas.
- **Independencia de `external_agents`**: La nueva implementación reside completamente dentro de `core/agents/` y no depende de la carpeta `external_agents/open_deep_research`, la cual será eliminada.