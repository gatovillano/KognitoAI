...
const TelegramLoginWidget = ({ onLogin, isProcessing }: { onLogin: (token: string) => Promise<void>; isProcessing: boolean }) => {
...
const EmailLoginForm = ({ onLogin, setParentError, isProcessing }: { onLogin: (token: string) => Promise<void>; setParentError: (error: string) => void; isProcessing: boolean }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Button type="submit" className="w-full h-12 rounded-2xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white font-medium shadow-lg hover:shadow-xl transition-all duration-200" disabled={isSubmitting || isProcessing}>
        {isSubmitting || isProcessing ? 'Ingresando...' : 'Iniciar Sesión con Email'}
      </Button>
    </form>
  );
};
                ...
                {view.type === 'login' && (
                  <div className="space-y-4">
                    ...
                    <div className="relative">
                     ...
                     <EmailLoginForm onLogin={handleSuccessfulLogin} setParentError={setGlobalError} isProcessing={isProcessing} />
