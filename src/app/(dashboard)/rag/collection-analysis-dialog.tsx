...
import { Expand, HelpCircle, FileText, Network, Lightbulb, Link, Search, BarChart3, AlertTriangle } from 'lucide-react';
    ...
    <>
      <Dialog open={isOpen} onOpenChange={onOpenChange}>
        ...
        <DialogContent className="max-w-5xl w-full max-h-[90vh] rounded-3xl backdrop-blur-xl bg-card/95 border-0 shadow-2xl flex flex-col p-0">
          ...
          <Tabs defaultValue="overview" className="flex-1 flex flex-col overflow-hidden">
            <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 text-xs bg-transparent border-b border-border/50 mb-4 px-6">
            ...
            <ScrollArea className="flex-1 p-6 pt-0">
              <div className="space-y-6 pb-4">
                ...
                {/* Nueva Pestaña de Problemáticas */}
                <TabsContent value="problematic_areas" className="space-y-3">
                  <Card className="border-none shadow-none bg-transparent p-0">
                    <CardHeader className="px-0 pt-0 pb-2">
                      <CardTitle className="flex items-center gap-2 text-lg font-semibold text-foreground">
                    ...
                    <CardContent className="p-0">
                      {mappedAnalysis.problematic_areas && mappedAnalysis.problematic_areas.length > 0 ? (
                        <div className="space-y-3">
                          {mappedAnalysis.problematic_areas.map((problem: string, index: number) => (
                            <div key={index} className="p-3 bg-red-50 border-l-4 border-red-200 rounded-r-lg border border-border/50">
                              <InlineMarkdownRenderer content={problem} />
                            </div>
                          ))}
                        </div>
                      ) : (
      ...
      <Dialog open={isThemeDialogOpen} onOpenChange={setIsThemeDialogOpen}>
        <DialogContent className="max-w-xl w-full max-h-[90vh] rounded-3xl backdrop-blur-xl bg-card/95 border-0 shadow-2xl flex flex-col p-0">
                      ...
                      {selectedTheme && (            <ScrollArea className="flex-1 pr-4">
              <div className="space-y-4 pb-4">
                <div>
                  <h4 className="font-semibold text-lg mb-2 text-foreground">
                    {typeof selectedTheme === 'string' ? selectedTheme : selectedTheme.theme}
                  </h4>
                </div>
