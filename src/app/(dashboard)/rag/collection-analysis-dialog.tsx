        ...
        <ScrollArea className="max-h-[70vh] pr-4">
            <div className="space-y-6">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground p-3 bg-muted rounded-md whitespace-pre-wrap">{analysis.resumen_general_coleccion}</p>
                </div>
                <div>
                    <h3 className="font-semibold mb-2">Temas Transversales</h3>
                    <div className="flex flex-wrap gap-2">
                        {analysis.temas_transversales?.map((t: string) => <Badge key={t}>{t}</Badge>)}
                    </div>
                </div>
                <div>
                    <h3 className="font-semibold mb-2">Conexiones Identificadas</h3>
                    <div className="space-y-3">
                       {analysis.conexiones_identificadas?.map((conn: any, i: number) => (
                 ...
                 <div>
                    ...
                    <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground">
                       {analysis.brechas_conocimiento?.map((gap: string, i: number) => <li key={i}>{gap}</li>)}
                    </ul>
                </div>
            </div>
        </ScrollArea>
