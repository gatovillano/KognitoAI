...
export default function CollectionDetailPage() {
  ...
  const fetchPageData = useCallback(async () => {
    ...
    try {
      ...
      const filteredDocs = docsRes.data.filter((doc: Document) => doc.topic === topic);
    ...
    const poller = setInterval(async () => {
      try {
        ...
        if (status === 'completed') {
          ...
          clearInterval(poller); setDocPollingId(null); toast.error("El análisis del documento falló.", { description: error });
        }
    ...
    const poller = setInterval(async () => {
      try {
        ...
        if (status === 'completed') {
          ...
          clearInterval(poller); setCollectionPollingId(null); toast.error("El análisis de la colección falló.", { description: error });
        }
