...
class DocumentRAGTool(BaseTool):
    ...
    async def _arun(
        ...
        logger.info(f"💾 Calling process_document_for_rag for user {self.account_id}, file '{file_name}', topic '{topic}', metadata: {metadata_to_pass}")
        ...
        try:
            ...
            if chunks_count > 0:
                ...
                source = Source(id=1, title=file_name, url="", snippet=extracted_text)  # Aquí asignamos los valores
                return ToolOutputWithSources(
                    context_for_llm=f"Document '{file_name}' processed and {chunks_count} chunks added to your knowledge base under the topic '{topic}'. You can now ask me questions about this document.",
                    sources=[source]
                )
            else:
                logger.error(f"❌ RAG processing failed for {file_name}. No chunks added.")
                return ToolOutputWithSources(context_for_llm=f"Error processing document '{file_name}'. Could not add its content to your knowledge base. Please check the logs.", sources=[])

        except Exception as e:
            logger.error(f"❌ Error in DocumentRAGTool for user {self.account_id}: {e}", exc_info=True)
            return ToolOutputWithSources(context_for_llm=f"An error occurred while processing the document: {e}", sources=[])

    def _run(self, **kwargs: Any) -> str:
        """Synchronous _run method is not supported."""
